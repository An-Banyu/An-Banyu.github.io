...
(function(){
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Observe DOM mutations so that when Twikoo (or third-party) rewrites inline style or src we reapply
    const observer = new MutationObserver((records) => {
      records.forEach(rec => {
        ...
        if (rec.type === 'attributes' && (rec.attributeName === 'style' || rec.attributeName === 'class' || rec.attributeName === 'src')) {
          ...
          if (target && target instanceof Element) {
            ...
            // if an avatar img had its src changed by Twikoo, reapply from last entered email when possible
            if (rec.attributeName === 'src') {
              try {
                const avatarSelectors = ['.tk-avatar img', '.tk-avatar-img', 'img.tk-avatar-img', '.twikoo-avatar img'];
                if (avatarSelectors.some(s => target.matches && target.matches(s))) {
                  if (typeof lastEmail !== 'undefined' && lastEmail) {
                    const newSrc = emailToAvatar(lastEmail);
    ...
    const updateAvatarPreviewFromEmail = (val) => {
      ...
      lastEmail = val ? String(val).trim().toLowerCase() : null;
      if(!avatarUrl) return;
      // try form preview avatars inside twikoo editor
      const previewSelectors = ['#twikoo-wrap .tk-editor .tk-avatar img', '#twikoo-wrap .tk-avatar img', '.tk-editor .tk-avatar img', '.twikoo-avatar img', '.tk-avatar-img', 'img.tk-avatar-img'];
